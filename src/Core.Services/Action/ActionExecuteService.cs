...
namespace Core.Services.Action
{
    public class ActionExecuteService(IDbContextFactory<ApplicationDbContext> _dbContextFactory, DataService _dataService)
    {
        public async Task ExecuteActionAsync(string caseName, string actionName, object? calculatedData = null)
        {
            ...
            var allEntries = (await _dataService.GetDataEntriesAsync(db, caseName, null, actionDef.TagUsedInAction.ToArray(), actionDef.DataSetUsedInAction.ToArray())).ToList();
            ...
            foreach (var dsName in actionDef.DataSetUsedInAction)
            {
                ...
                var entriesBySet = allEntries.Where(e => e.DataSet != null && e.DataSet.Name.Equals(dsName, StringComparison.OrdinalIgnoreCase)).ToList();

                foreach (var entry in entriesBySet)
                    engine.SetValue($"set_{entry.DataDefinition.Name}", entry);
            }
            ...
            {
                await _dataService.UpdateDataEntryAsync(caseName, entry.Id, entry.Values, entry.Tags?.Select(t => t.Name).ToList() ?? null, entry.DataSet?.Name ?? null);
            }
        }
    }
}
